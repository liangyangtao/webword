...
$.ajaxSetup({   
	...
	complete:function(XMLHttpRequest,textStatus){
	    ...
	    if(sessionstatus=='reload'){//如果超时就处理 ，指定要跳转的页面    
	    	...
	    	window.location.href=path+"view/home.do"; //跳转到他人登录页面   
	    }
	}
}); 
...
function show_save_success(){
	...
	window.setTimeout("hide_save_success()", 3000);
}
function hide_save_success(){$('.ui_back2,.ui_back,.box_content,.save_success').hide();}
...
//	}	
	if($('.add_channel').length>0){
		//我的栏目，新建栏目，联想框>全部标签
		var ss2 =  new oSearchSuggest($('.add_channel .sb_middle input#mustTagNames'),$('.add_channel .sb_middle'),'174','122',$('.add_channel'),5,1);
		//我的栏目，新建栏目，联想框>任意标签
		var ss3 =  new oSearchSuggest($('.add_channel .sb_middle input#shouldTagNames'),$('.add_channel .sb_middle'),'174','160',$('.add_channel'),5,1);
		//我的栏目，新建栏目，联想框>不包含标签
		var ss3 =  new oSearchSuggest($('.add_channel .sb_middle input#mustNotTagNames'),$('.add_channel .sb_middle'),'174','197',$('.add_channel'),5,1);
		//我的栏目，新建栏目，联想框>全部关键词
		var ss4 =  new oSearchSuggest($('.add_channel .sb_middle input#mustWordNames'),$('.add_channel .sb_middle'),'174','122',$('.add_channel'),5,2);
		//我的栏目，新建栏目，联想框>任意关键词
		var ss5 =  new oSearchSuggest($('.add_channel .sb_middle input#shouldWordNames'),$('.add_channel .sb_middle'),'174','160',$('.add_channel'),5,2);
		//我的栏目，新建栏目，联想框>不包含关键词
		var ss6 =  new oSearchSuggest($('.add_channel .sb_middle input#mustNotWordNames'),$('.add_channel .sb_middle'),'174','197',$('.add_channel'),5,2);
	}
...
$('.previ_channel .sb_button .subscribe,.previ_channel .sb_button a.a3').click(function(){	
...
var bc,ac;
/* 向右移动 */
$(".news_sourse .sb_middle div.divm img.add").click(function(){
			...
			if($(this).find("input").attr("checked")=="checked"){
				...
				for(i=0;i<bc.length;i++){								
				 ...
				 if(sco==0){//判断右侧没有相同的 新建个
					...
					var str1="<ul style='display:block' tid='"+_this.parent().attr("tid")+"'><span class='tree1'><span><i></i><input type='checkbox' checked='checked' /><ol class='ol1'>"+_this.parent().find(".ol1").text()+"</ol></span></span></ul>";
...
/* 向左移动 */
$(".news_sourse .sb_middle div.divm img.remove").click(function(){		
...
function highSearch(selfThat,isLeSear){
	...
	$.ajax({
	   ...
	   url: "/webword/homePage/searchNewsSource.do?newsName=&moduleName=&noIds=1,2,3",
	   dataType:"json",
	   success: function(result){
		   var html='';		     
		   if(selfThat!=''){			  
			    var self_text;			    
				self_text=selfThat.split('|');				
				var selfVal=$(".add_channel #selected_sour_id").val();
				//console.log(selfThat+''+selfVal);
				selfVal=selfVal.split(',');				
				if(selfVal.length>0){
						for(var j=0;j<selfVal.length;j++){
							for(var i = 0; i < result.length; i++){
								if(result[i].id==selfVal[j]){									
									var str1="<ul style='display:block' tid='"+result[i].id+"'><span class='tree1'><span><i></i><input type='checkbox' checked='checked' /><ol class='ol1'>"+result[i].sourcename+"</ol></span></span></ul>";
									$(".news_sourse .sb_middle .div2 .over_scroll").append(str1);
									//result.splice(i,1);
		   ...
		   for(var i = 0; i < result.length; i++){			   
				if(hiddenId.length>0 ){					
					for(var j = 0; j < hiddenId.length; j++){
						if( result[i].id == hiddenId[j]){
							//console.log(result[i].id);
							html+='<ul tid="'+result[i].id+'" style="display:none"><span class="tree1"><span><i class="i_down"></i><input type="checkbox" /><ol class="ol1">'+result[i].sourcename+'</ol></span></span></ul>';	
							result.splice(i,1);
						}
					}					
				}
				
				html+='<ul tid="'+result[i].id+'"><span class="tree1"><span><i class="i_down"></i><input type="checkbox" /><ol class="ol1">'+result[i].sourcename+'</ol></span></span></ul>';
...
$('.news_sourse #LeSeaBox a#LeSearch').click(function(){highSearch('','yes');});
$('.news_sourse #RiSeaBox a#RiSearch').click(function(){//右侧已选，内部筛选
	bc=$(".news_sourse .sb_middle .div2").find(".ol1");
	var str=$('#RiSeaBox input').eq(0).val();
	for(i=0;i<bc.length;i++){
...
/* 弹出  选择新闻源 */
$(".add_channel #select_sources").click(function(){
	$(this).blur();
...
$(".modifi_channel #select_sources2").click(function(){	
	...
	$(".selected_sourse,.box_content,.ui_back,.ui_back2").fadeIn(100);
	 var self_text = $(".modifi_channel #select_sources2").text().split('|');				
	 var selfVal   = $(".modifi_channel #selected_sour_id2").val().split(',');	
	 //console.log(self_text);
	 var html=''
	 for(var i=0;i<self_text.length;i++){		 
		 html+="<ul style='display:block' tid='"+selfVal[i]+"'><span class='tree1'><span><i></i><input type='checkbox' checked='checked' style='display:none' /><ol class='ol1'>"+self_text[i]+"</ol></span></span></ul>";
	 }	 
	 $(".selected_sourse .sb_middle .div2 .over_scroll").append(html);
})
/* 新闻源确认 */
$(".news_sourse .sb_button .a1,.news_sourse .pop_closeBt_s").click(function(){	
	var textArr=$('.news_sourse .sb_middle .div2 .over_scroll ul .ol1');	
	var str=str1='';
	var idr=idr1='';		
	for(var i=0;i<textArr.length;i++){
	str = textArr.eq(i).text()+'|';
	...
	$('.add_channel input#selected_sour_id').val(idr1);
...
$('.selected_sourse #RiSeaBox2 a#RiSearch2').click(function(){//右侧已选，内部筛选
	...
	var str=$('#RiSeaBox2 input').eq(0).val();
	for(i=0;i<bc.length;i++){
		var flag = bc.eq(i).text().indexOf(str);
		if(flag=='-1'){
			bc.eq(i).parent().parent().parent().hide();
