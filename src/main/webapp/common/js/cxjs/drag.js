...
function mouseup(e){
	...
	if(dragobj.omflag){//存在插入框
		...
		if(dragobj.locationNode!=null&&dragobj.locationNode!=""){
			var firstId=dragobj.firstNode.id.split("_")[1];
			var loactionId =dragobj.locationNode.id.split("_")[1];
			//console.info("firstId="+firstId+",location="+dragobj.location+",loactionId="+loactionId);
			updateOrder(firstId,dragobj.location,loactionId);
		}else{
			//console.info("不处理");
		}
	}else{
		...
		//console.info("删除元素");
		plateDel(null,dragobj.firstNode,dragobj.firstNode.id.split("_")[1]);
	}
...
function moveList(e){
	...
	for(var i=0;i<nodelist.length;i++){
        ...
        //console.info("pnode="+pnode.getAttribute("name"));
 		...
 		if(pnode.innerHTML==""||pnode.style.display=="none") {
 			//console.info("----------"+pnode.id);
 			//插入层 隐藏层，不遍历
 			continue;
 		}
 		//console.info("+++++++++++"+pnode.id);
...
function pmousedown(e,obj){
	...
	if(clickObj.nodeName!="IMG"){
		return false;
	}
	if(obj.parentNode.getElementsByTagName("li").length<2){
		return false;
	}
	document.onmouseup=mouseup;
	if(dragobj.dragElement!=null) return false;
	G_plateId=obj.id.split("_")[1];
	...
	dragnode.className = "list_ul2";
	var newli = obj.cloneNode(true);
	dragnode.appendChild(newli);
	document.body.appendChild(dragnode);
	dragobj.dragNode = dragnode;
	var dragDiv = ZU.$id("displayUl1");
