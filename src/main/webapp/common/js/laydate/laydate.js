...
!function (a) {
    ...
    c.check = function () {
        ...
        if ('' !== d.replace(/\s/g, '')) {
            ...
            0 | e[1],
            0 | e[2]], 1)  : f.value !== c.elem[c.elemv] && (c.elem[c.elemv] = f.value)
        }
    },
    c.months = [
        31,
    ...
    c.viewDate = function (a, b, d) {
        ...
        f.times = [
            0 | c.inymd[3] || 0 | c.hmsin[0].value,
            0 | c.inymd[4] || 0 | c.hmsin[1].value,
            0 | c.inymd[5] || 0 | c.hmsin[2].value
        ],
    ...
    c.initDate = function () {
        ...
        e.length < 3 && (c.options.start = c.options.start || '', e = c.options.start.match(/\d+/g) || [], e.length < 3 && (e = [
            d.getFullYear(),
            d.getMonth() + 1,
            d.getDate()
        ])),
    ...
    c.events = function () {
        ...
        c.each(b('#laydate_table td'), function (a, b) {
            c.on(b, 'click', function (a) {
				...
				function checkTime(id1,id2){
					stratTimeVal = $(id1).val();
					endTimeVal = $(id2).val();								
					if(stratTimeVal&&endTimeVal&&stratTimeVal.length>0&&endTimeVal.length>0){		
						var todaystr=laydate.now();						
						if(stratTimeVal<endTimeVal){							
							alert("截止时间不能小于开始时间");
							$(id1).val('');	$(id2).val('');
						}
						if(stratTimeVal>todaystr){
							alert("截止时间不能大于今天");
							$(id1).val('');	$(id2).val('');							
						}
					}					
				}
				checkTime('#time_start','#time_end');
				checkTime('#time_start1','#time_end1');
            })
        }),
